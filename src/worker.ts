  ...
  async processMessages(message: Message, channel: Channel) {
    try {
      ...
      if (payloadObject.method === 'createApp') {
        ...
        let i = 0;
        do {
          const response = await this.googleAdmin?.createNewApp(payload?.displayName, payload?.bundleId, platform ?? '-');
          i++;
          switch (response?.code) {
            case 0:
              ...
              await model.save();
              this.logger.log(` [x] App bundleId: ${model?.bundleId} platfrm: ${platform} foi criado com sucesso`); channel.ack(message);
              return true;
            case 1:
              this.logger.warn(` [x] App bundleId: ${model?.bundleId} platfrm: ${platform} encontra-se criado`); channel.ack(message);
              return true;
            case -1:
              if (i < 4) {
                n += i;
        ...
        this.logger.error(
          `nao foi possivel o App bundleId: ${model?.bundleId} platfrm: ${platform} após ${i} tentativas em ${(startTime - new Date().getTime()) / 1000}s.`,
        );
      }
    } catch (error: any) {
      this.logger.error(error);
