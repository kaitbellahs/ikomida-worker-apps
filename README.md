...
The platform offered vendors their own branded application. Producing one is a long-running build, so it is queued rather than requested — this worker consumes those build jobs.

## Queue

Consumes `APPS_QUEUE` from RabbitMQ. Messages are processed with bounded retries and acknowledged only on success; failures are negatively acknowledged so nothing is lost silently.
